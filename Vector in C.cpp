...
int* make_num_arr(int& num_nums) {
	...
	while (am_exiting == false) {
		...
		if (input_num == -1) {
			...
			if (logical_size == physical_size) {             // this 'if' must happen before add input to array 
				expand_num_arr = new int[2 * physical_size];
				for (int i = 0; i < logical_size; i++) {    // copy over the num_arr contents to the exapanded array
					expand_num_arr[i] = num_arr[i];
				}
				delete[] num_arr;             // manage the memory
				num_arr = expand_num_arr;     // set array you are returning to the new array
